...
if ph < 7.2 or ph > 7.4:
    ...
    dif = abs(ph_hh - ph)
    st.write(f"pH Calculado (HH): {ph_hh:.3f} | Dif: {dif:.3f}")
    if dif <= 0.05: st.success("HAY CONSISTENCIA INTERNA")
    else: st.error("NO HAY CONSISTENCIA: Repetir toma.")
else:
...
# --- II. ESTADO ---
st.header("II. Estado pH")
if ph < 7.4: st.error("ACIDOSIS")
elif ph > 7.4: st.success("ALCALOSIS")
else: st.info("NEUTRO")

# --- III. TRASTORNOS ---
st.header("III. Trastornos Metabólicos y Respiratorios")
...
if ph < 7.4 and pco2 > 32:
    ...
    st.write("**Causas:** SNC (VITAMINS), SNP (Guillain Barré), Placa (Miastenia), Muscular (Fatiga), Alvéolo (EPOC/SDRA), Tórax.")
...
if ph > 7.4 and pco2 < 28:
    ...
    st.write("**Causas (VINDICATE):** Vascular, Infección, Neoplasia, Drogas, Idiopático, Congénito, Autoimmune, Trauma, Endocrino.")

# --- IV. OXIGENACIÓN ---
st.divider()
st.header("IV. Evaluación de la Oxigenación")

# Cálculo de Gradiente Alveolo-arterial (Fórmula Bogotá)
...
if pa02 < 60:
    ...
    st.markdown("**Causas:** 1-PB baja, 2-OVACE, 3-Laringe, 4-Tráquea/Bronquios, 5-Alvéolo (Agua/Pus/Sangre), 6-Intersticio, 7-Vaso (TEP).")

c1, c2, c3 = st.columns(3)
c1.metric("PAFI", f"{(pa02/fio2):.1f}")
c2.metric("SAFI", f"{(spo2/fio2):.1f}")
c3.metric("ROX Index", f"{((spo2/fio2)/fr):.2f}")
