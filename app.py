...
# Sidebar para registro y estadísticas
st.sidebar.header("📊 Registro Académico")
...
if abs(h_calc - h_ph) < 5:
    ...
    st.error(f"⚠️ Gases Inconsistentes. Revisar valores.")

# B. ANÁLISIS ÁCIDO-BASE (SEGÚN EXCEL)
st.header("🔬 Interpretación Ácido-Base")

ag_medido = na - (cl + hco3)
ag_corr = ag_medido + (2.5 * (4 - alb))

# FÓRMULA EXACTA DE LA IMAGEN (DELTA DEL GAP)
# Delta Gap = (AG Medido - 10) - (20 - HCO3 Medido)
delta_del_gap = (ag_corr - 10) - (20 - hco3)

if ph < 7.36:
    st.error("ESTADO: ACIDEMIA")
    if hco3 < 19: # Umbral metabólico Bogotá
        st.subheader("Trastorno Primario: ACIDOSIS METABÓLICA")
        
        # 1. Winter
        p_esp = (1.5 * hco3) + 8
        st.info(f"pCO2 esperada (Winter): {p_esp:.1f} (+/- 2)")
        
        # 2. Delta del Gap (Interpretación de la imagen)
        st.subheader("⚖️ Paso 6: Delta del Gap")
        st.metric("Resultado Delta del Gap", f"{delta_del_gap:.1f}")
        ...
        # 3. Anion Gap Corregido y Causas
        st.write(f"**Anion Gap Corregido:** {ag_corr:.1f}")
        if ag_corr > 12:
            st.warning("**Causas (GOLDMARCC):** Glicoles, Oxoproline, L-Lactato, D-Lactato, Metanol, Aspirina, Renal (Uremia), Cetoacidosis.")
        else:
            st.info("Sugerencia: Realizar Delta Gap Urinario (AG Normal/Bajo).")
    else:
...
grad_esp = (edad / 4) + 4
if grad > grad_esp:
    st.write(f"Interpretación: **Gradiente Elevado** (Normal para la edad: {grad_esp:.1f}).")
else:
    st.write("Interpretación: **Gradiente Normal**.")

st.divider()
st.write(f"**Estudiante:** {nombre_estudiante if nombre_estudiante else 'No registrado'}")
st.write(f"**ID:** {id_estudiante if id_estudiante else 'No registrado'}")
